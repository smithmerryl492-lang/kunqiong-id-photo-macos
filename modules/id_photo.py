...
class IDPhotoModule(BaseModule):
    ...
    def process(self, image_path: str, bg_color: str = 'white', size: str = 'no_crop', 
        ...
        if target_size:
            ...
            if face_info:
                # 优先使用用户启用的高级参数，否则用智能算法
                if self.advanced_params.get('enabled'):
                    head_measure_ratio = self.advanced_params['head_measure_ratio']
                    top_distance_max = self.advanced_params['top_distance_max']
                    top_distance_min = max(0.02, top_distance_max - 0.02)
                else:
                    # Step 1: 分析原图特征（拍摄类型）
                ...
                if self.advanced_params.get('enabled'):
                    print(f"\n[==========  高级参数（已启用）  ==========]")
                    print(f"head_measure_ratio: {head_measure_ratio:.3f}, top_distance: {top_distance_max:.3f} ~ {top_distance_min:.3f}")
                    print(f"[========================================]\n")
                else:
                    print(f"\n[==========  智能证件照参数  ==========]")
    ...
    def _show_advanced_dialog(self):
        ...
        dialog.setStyleSheet("""
            ...
            QCheckBox { color: hsl(213, 31%, 91%); font-size: 13px; spacing: 8px; }
            QCheckBox::indicator { width: 18px; height: 18px; border-radius: 3px; border: 2px solid hsl(217.2, 32.6%, 17.5%); background: hsl(224, 71.4%, 4.1%); }
            QCheckBox::indicator:checked { border-color: hsl(221.2, 83.2%, 53.3%); background: hsl(221.2, 83.2%, 53.3%); }
        """)
