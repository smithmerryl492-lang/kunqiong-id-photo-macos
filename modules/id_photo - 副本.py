...
class IDPhotoModule(BaseModule):
    ...
    def __init__(self):
        ...
        self.bg_colors = {
            ...
            '灰色': (200, 200, 200),             # 浅灰色
            '美式证件照': 'american_style',    # 美式证件照特殊背景
            '自定义(RGB)': 'custom_rgb',        # 自定义RGB
            '自定义(HEX)': 'custom_hex',        # 自定义HEX
        }
    ...
    def create_print_layout(self, photo_path: str, paper_type: str = '6寸', 
        ...
        # 如果指定了数量，重新计算最优行列数组合（参考HivisionIDPhotos的做法）
        if count is not None and count < auto_count:
            # 根据指定数量，计算最优的行列数组合
            # 优先选择接近正方形的布局（行列数接近），同时确保不超出可用区域
    ...
    def get_ui_widget(self) -> QWidget:
        ...
        self.bg_combo.currentTextChanged.connect(self._on_bg_color_changed)
    ...
    def _on_bg_color_changed(self, color_name):
        """背景颜色变更回调"""
        if color_name == '自定义(RGB)':
            self._show_custom_rgb_dialog()
        elif color_name == '自定义(HEX)':
            self._show_custom_hex_dialog()
    
    def _show_custom_rgb_dialog(self):
        """显示自定义RGB颜色对话框"""
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QWidget, QMessageBox
        from PyQt6.QtGui import QIntValidator
        
        dialog = QDialog()
        dialog.setWindowTitle("自定义RGB颜色")
        dialog.setMinimumWidth(350)
        dialog.setStyleSheet("""
            QDialog { background-color: hsl(222.2, 84%, 4.9%); color: hsl(213, 31%, 91%); }
            QLineEdit { background-color: hsl(224, 71.4%, 4.1%); border: 1px solid hsl(217.2, 32.6%, 17.5%); 
                       border-radius: 4px; padding: 8px; color: hsl(213, 31%, 91%); }
        """)
        ...
        # RGB输入
        r_edit = QLineEdit(str(self.custom_color[2]))  # BGR -> RGB
        r_edit.setValidator(QIntValidator(0, 255))
        g_edit = QLineEdit(str(self.custom_color[1]))
        g_edit.setValidator(QIntValidator(0, 255))
        b_edit = QLineEdit(str(self.custom_color[0]))  # BGR -> RGB
        b_edit.setValidator(QIntValidator(0, 255))
        
        for label, edit in [('R:', r_edit), ('G:', g_edit), ('B:', b_edit)]:
            row = QWidget()
        ...
        def confirm():
            try:
                r, g, b = int(r_edit.text()), int(g_edit.text()), int(b_edit.text())
                self.custom_color = (b, g, r)  # 存为BGR
                dialog.accept()
            except:
                QMessageBox.warning(dialog, "错误", "请输入有效的RGB值 (0-255)")
        btn.clicked.connect(confirm)
        layout.addWidget(btn)
        dialog.exec()
    
    def _show_custom_hex_dialog(self):
        """显示自定义HEX颜色对话框（官方）"""
        from PyQt6.QtWidgets import QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QMessageBox
        
        dialog = QDialog()
        dialog.setWindowTitle("自定义HEX颜色")
        dialog.setMinimumWidth(350)
        dialog.setStyleSheet("""
            QDialog { background-color: hsl(222.2, 84%, 4.9%); color: hsl(213, 31%, 91%); }
            QLineEdit { background-color: hsl(224, 71.4%, 4.1%); border: 1px solid hsl(217.2, 32.6%, 17.5%); 
        ...
        layout.addWidget(QLabel("HEX颜色值:"))
        hex_edit = QLineEdit()
        hex_edit.setPlaceholderText("#FFFFFF")
        # BGR转RGB再转HEX
        current_hex = '#{:02X}{:02X}{:02X}'.format(self.custom_color[2], self.custom_color[1], self.custom_color[0])
        hex_edit.setText(current_hex)
        layout.addWidget(hex_edit)
        
        btn = QPushButton("确定")
        def confirm():
            hex_val = hex_edit.text().strip().lstrip('#')
            try:
                if len(hex_val) == 6:
                ...
                QMessageBox.warning(dialog, "错误", "请输入有效的HEX颜色值 (e.g., #FFFFFF)")
        btn.clicked.connect(confirm)
        layout.addWidget(btn)
        dialog.exec()
    ...
    def _apply_image_watermark(self, img_rgba: np.ndarray) -> np.ndarray:
        ...
        if watermark is None:
            print(f"[水印] 无法读取水印图片: {image_path}")
            return img_rgba
